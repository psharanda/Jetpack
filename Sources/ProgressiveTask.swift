...
public struct ProgressiveTask<T, ProgressType> {
    ...
    private let worker: (@escaping (ProgressType) -> Void, @escaping (Result<T>) -> Void) -> Disposable
    
    /**
     Create task with worker
     */
    public init(worker: @escaping (@escaping (ProgressType) -> Void, @escaping (Result<T>) -> Void) -> Disposable) {
        self.worker = worker
    }
     ...
     */
    @discardableResult
    public func start(progress: @escaping (ProgressType) -> Void, completion: @escaping (Result<T>) -> Void) -> Disposable {
...
extension ProgressiveTask: TaskProtocol {
     ...
     */
    @discardableResult
    public func start(_ completion: @escaping (Result<T>) -> Void) -> Disposable {
...
extension ProgressiveTask {
     ...
     */
    public init(workerQueue: DispatchQueue, completionQueue: DispatchQueue = .main, worker: @escaping (@escaping (ProgressType) -> Void) -> Result<T>) {
    ...
    public init(workerQueue: DispatchQueue, completionQueue: DispatchQueue = .main, worker: @escaping (@escaping (ProgressType) -> Void) -> T) {
        self.init(workerQueue: workerQueue, completionQueue: completionQueue) { (progress) -> Result<T> in
